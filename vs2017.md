...
for %i in (bzip2 curl freetype gtest libjpeg-turbo openssl libpng zlib) do vcpkg install %i:x64-windows-static
```

<!--
for %i in (bzip2 curl freetype gtest libjpeg-turbo openssl libpng zlib) do vcpkg install %i:x64-windows-static %i:x86-windows-static-xp
-->

Add `C:\Libraries\vcpkg` to the `PATH` environment variable.<br/>
Set `C:/Libraries/vcpkg` as the `VCPKG_PATH` environment variable.<br/>
Add `-DCMAKE_TOOLCHAIN_FILE=%VCPKG_PATH%/scripts/buildsystems/vcpkg.cmake` to CMake options.
